...
**Skills:** SELECT, WHERE
**Why these skills:** WHERE clause filters records based on specific criteria, essential for targeted business analysis
**Query:**
...
**Conclusion:** 4 customers have balances above the company average ($432.35), totaling $3,869.53. These premium customers represent 40% of the customer base but account for 89% of outstanding receivables, indicating they may be high-value clients with extended credit terms.
Smith     | Olette    | 453.98
...
**Why these skills:** GROUP BY aggregates data by destination, COUNT() provides trip totals, aliasing makes output readable
**Query:**
...
**Why these skills:** GROUP BY aggregates by driver, HAVING filters grouped results (vs WHERE), COUNT() measures activity level
**Query:**
...
**Why these skills:** ORDER BY DESC sorts by distance (largest first), LIMIT restricts to top 5 results for focused analysis
**Query:**
SELECT TRIP_ID, TRIP_DRIVER, TRIP_DESTINATION, TRIP_DISTANCE
FROM TRIP 
...
**Why these skills:** INNER JOIN connects multiple tables (TRIP, EMPLOYEE, CUSTOMER) to show relationships, aliasing simplifies table references and output readability
**Query:**
...
**Why these skills:** LEFT JOIN includes all employees (even non-drivers), CASE statement handles NULL values by providing readable status labels
**Query:**
...
**Why these skills:** UNION combines different table results into single dataset, CONCAT creates full names, useful for emergency contact lists and company-wide notifications
**Query:**
...
**Why these skills:** CASE statement creates conditional logic to categorize continuous data (distance) into discrete business categories
**Query:**
...
**Skills:** Subqueries
**Why these skills:** Subquery first identifies long-distance trips, then outer query finds which drivers handle them, useful for analyzing specialization
**Query:**
...
**Skills:** Subqueries with Aggregates
**Query:**
SELECT CUS_LNAME, CUS_FNAME, CUS_BALANCE
FROM CUSTOMER 
...
**Why these skills:** ROW_NUMBER() assigns unique ranks, RANK() handles ties, OVER clause defines ranking criteria - useful for performance analysis
**Query:**
...
**Why these skills:** PARTITION BY groups calculations by driver, running totals show cumulative workload, averages show per-driver performance metrics
**Query:**
SELECT TRIP_ID, TRIP_DRIVER, TRIP_DISTANCE,
...
**Why these skills:** CTEs break complex analysis into readable steps - first calculate stats, then add rankings, finally join with names for business context
**Query:**
WITH DriverStats AS (
  SELECT TRIP_DRIVER, 
         COUNT(*) AS TripCount, 
...
**Why these skills:** DATE_FORMAT() extracts month from dates, CTEs organize the analysis steps, window functions calculate cumulative metrics over time
**Query:**
...
**Conclusion:** February 2024 shows strong operational activity with 18 trips covering 13,414 total miles. The average trip distance of 745.22 miles indicates a healthy mix of medium to long-distance operations, positioning the company well for sustained revenue generation.
...
This fleet management database demonstrates how strategic data analysis transforms operational metrics into actionable business intelligence, enabling informed decision-making across all aspects of the transportation business.